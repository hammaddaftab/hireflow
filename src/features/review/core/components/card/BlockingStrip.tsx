...
function ExperienceRow({ experience }: { experience: EvaluatedExperienceRequirement }) {
  return (
...
function SkillsRow({ skills }: { skills: EvaluatedSkillsRequirement }) {
  const {
    statedSkills,
    notStatedSkills,
    totalSkillsCount,
    ...
    <div className="space-y-2">
      ...
      <div className="pt-1.5 flex flex-wrap items-center gap-2 border-t border-outline-variant/20">
          ...
          content={
            <div className="text-xs space-y-1.5 max-w-xs p-0.5">
              <div className="flex items-center justify-between gap-2 font-bold text-on-surface">
                ...
                <span
                  className={`text-[10px] font-mono px-1.5 py-0.5 rounded font-bold ${getBadgeStyles(
                    derived.unverifiedClaimsDotType
                  )}`}
                >
                  {derived.unverifiedClaimsBadgeText}
                </span>
              </div>
              <div className="text-[11px] text-on-surface-variant">
                {orphanSkillsCount === 0
        ...
        >
          ...
          >
            <EvidentiaryDot type={derived.unverifiedClaimsDotType} />
            <span>{derived.unverifiedClaimsPillText}</span>
          </span>
        </Tooltip>
      </div>
    </div>
...
function EducationRow({ education }: { education: EvaluatedEducationRequirement }) {
  return (
    <div className="space-y-1.5">
      <div className="text-[10px] font-bold uppercase tracking-wider text-on-surface-variant">
        education
...
function LogisticsRow({ logistics }: { logistics: EvaluatedLogisticsRequirement }) {
  const {
    compensation,
    noticePeriod,
    workMode,
    ...
    <div className="space-y-1.5">
      ...
      <div className="flex flex-wrap items-center gap-2">
          ...
          content={
            <div className="text-xs space-y-1.5 max-w-xs p-0.5">
              <div className="flex items-center justify-between gap-2 font-bold text-on-surface">
                ...
                <span
                  className={`text-[10px] font-mono px-1.5 py-0.5 rounded font-bold ${getBadgeStyles(
                    derived.statusDotType
                  )}`}
                >
                  {derived.statusBadgeText}
                </span>
              </div>
              <div className="text-[11px] text-on-surface-variant">
        ...
        >
          ...
          >
            <EvidentiaryDot type={derived.statusDotType} />
            <span>{derived.statusPillText}</span>
          </span>
        </Tooltip>
...
export function BlockingStrip({
  ...
  return (
    <div className="w-full space-y-3.5 text-xs">
      ...
      {/* Category 2: Skills */}
      {skills && <SkillsRow skills={skills} />}

      {/* Category 3: Education */}
      {education && education.hasEducation && <EducationRow education={education} />}

      {/* Category 4: Logistics */}
      {logistics && <LogisticsRow logistics={logistics} />}
    </div>
  );
}
